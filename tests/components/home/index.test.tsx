...
describe('Home Component', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('renders introduction paragraph with correct content', () => {
      render(<Home />);
      const introText = screen.getByText(/I'm a Software Engineer with 5\+ years of experience/);
      ...
      expect(introText.textContent).toContain('PT. XL Axiata');
    ...
    it('renders Download CV button with icon', () => {
      ...
      expect(downloadButton.closest('a')?.getAttribute('href')).toBe('/document/CV-Ihsan-Nurul-Habib.pdf');
  ...
  describe('User Interactions', () => {
    ...
    it('Download CV button has correct download attributes', () => {
      ...
      expect(downloadLink?.getAttribute('href')).toBe('/document/CV-Ihsan-Nurul-Habib.pdf');
  ...
  describe('Styling and Layout', () => {
    it('has correct container and layout classes', () => {
      ...
      const flexContainer = screen.getByText('Ihsan Nurul Habib').closest('.flex.flex-col.items-center');
      expect(flexContainer).not.toBeNull();
    });

    it('has correct button container layout', () => {
      render(<Home />);
      const buttonContainer = screen.getByText('Connect with me').closest('div');
    ...
    it('applies hover and transition classes to buttons', () => {
      ...
      expect(connectButton.classList.contains('hover:shadow-accent/50')).toBe(true);
      ...
      expect(downloadButton?.classList.contains('hover:text-primary')).toBe(true);
      ...
      expect(downloadButton?.classList.contains('hover:shadow-accent/50')).toBe(true);
  ...
  describe('Icon Integration', () => {
    it('renders download icon in CV button', () => {
      ...
      expect(downloadButton?.querySelector('svg') || downloadButton?.querySelector('[data-testid="download-icon"]')).toBeTruthy();
    });
  });
});
